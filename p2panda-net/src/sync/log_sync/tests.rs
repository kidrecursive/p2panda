...
/// square-tower fork addition (D3-u fix, M4-21): when the ACCEPTOR of a session (not the peer who
/// originally initiated it) is the one whose own resync decision closes it -- D3-u's association
/// fires on whichever side actually holds the changed association, and that can be either side of
/// a session -- the ORIGINAL INITIATOR must recover on its own, not wait for `sync.resync_interval`'s
/// 30s timer. See `SURVIVOR_REINITIATE_DELAY`'s doc comment
/// (`p2panda-net/src/sync/actors/topic_manager.rs`) for the mechanism.
///
/// Mutation-proof: removing the surviving side's own re-`Initiate` (i.e. reverting to only the
/// closing side's `resume_pending_resync`) makes alice's session never reappear within this test's
/// bounded wait, since bob (the acceptor, in this harness never `Initiate`s himself and so is never
/// in his own `active_sync_set`) can't run `resume_pending_resync` for a session he never
/// `Initiate`-tracked either -- both this fix's paths are needed together for this exact topology.
#[tokio::test]
async fn acceptor_close_is_recovered_by_the_original_initiator() {
    setup_logging();

    let topic: Topic = [94; 32].into();
    let existing_log_id = 0;
    let new_log_id = 1;

    let mut bob = TestNode::spawn([89; 32], None).await;
    let alice = TestNode::spawn([90; 32], Some(bob.node_info())).await;
    ...
    // Alice initiates -- she is the original initiator, bob is the acceptor.
    alice_handle.initiate_session(bob.node_id());
    ...
    // Bob (the acceptor) associates a new log -- real drift from HIS OWN perspective -- then
    // resyncs (standing in for his own event-driven/timer resync check finding it).
    bob.client
        .create_operation(b"bob's new log", new_log_id)
        .await;
    bob.client
        .associate(
            &topic,
            &HashMap::from([(bob.client_id(), vec![new_log_id])]),
        )
        .await;
    bob_handle.resync(alice.node_id());

    // The ORIGINAL INITIATOR (alice) must recover on her own within well under
    // `sync.resync_interval`'s 30s default.
    let new_session_id = tokio::time::timeout(Duration::from_secs(2), async {
        loop {
            let event = alice_subscription.next().await.unwrap().unwrap();
            if event.session_id != session_id && matches!(event.event, Event::SyncStarted { .. }) {
                return event.session_id;
            }
        }
    })
    .await;
    assert!(
        new_session_id.is_ok(),
        "the original initiator must recover within 2s of the acceptor's own resync closing the \
         session, but got: {new_session_id:?}"
    );
}
